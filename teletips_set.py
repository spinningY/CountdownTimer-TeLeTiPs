...
async def set_timer(client, message):
    ...
    try:
        if message.chat.id>0:
            return await message.reply('⛔️ Coba perintah ini dalam **obrolan grup**.')
        elif not (await client.get_chat_member(message.chat.id,message.from_user.id)).privileges:
            return await message.reply('👮🏻‍♂️ Maaf, **hanya admin** yang dapat menjalankan perintah ini.')    
            ...
            if 0<user_input_time<=10:
                while user_input_time and not stoptimer:
                    ...
                    user_input_time -=1
                await finish_countdown.edit("🚨 Berbunyi! Berbunyi!! **WAKTUNYA HABIS!!!**")
                ...
                await get_user_input_time.edit(f"🤷🏻‍♂️ Saya tidak bisa menghitung mundur dari {user_input_time}")
...
async def stop_timer(Client, message):
    ...
    try:
        if (await bot.get_chat_member(message.chat.id,message.from_user.id)).privileges:
            stoptimer = True
            await message.reply('🛑 Hitung mundur berhenti.')
        else:
            await message.reply('👮🏻‍♂️ Maaf, **hanya admin** yang dapat menjalankan perintah ini.')
    except FloodWait as e:
        await asyncio.sleep(e.value)
